...
- `tests/types.js`: runs `tsc --checkJs` on `src/sim/` through `tsconfig.sim.json`, and fails on any error. It skips, with a message that says so, if TypeScript is not installed. Fast.
- `tsconfig.sim.json`: the type check for `src/sim/`. No DOM in its `lib`, which proves the rule that the sim core has no DOM. Zero errors today.
- `tsconfig.ui.json`: the type check for `src/ui/`, loaded together with `src/sim/` because the two share one scope in the built page. Not yet at zero errors; see `types/ui/`.
- `types/sim/`, `types/ui/`: JSDoc types are not written into `src/`, because `build.js` copies comments verbatim into `dist/hearth-sim.html`, and these two folders hold the shapes instead, in `.d.ts` files with no `import` or `export`, so they join the same global scope as `src/sim/` and `src/ui/`. A few functions in `src/sim/` still carry one JSDoc line each, `@returns` or `@type`, because there was no way to say their shape from outside the function; each is comment-only and marked as such in the type-checks proposal record.
