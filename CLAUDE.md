...
- `setClock(api, at)` in `tests/lib/run.js` is the one function that sets the world's date. A test may jump to a date rather than pay the days to reach it. It lives in the tests and never in `src/sim/`, because a rule reachable only through a back door in the product is a rule the product can lose without a sound. It writes two things: the world's `tick`, and on every being the tick its body was last brought up to (`seen`). The second write is why it must be used rather than assigning `api.tick`: a being's needs, warmth and hp are worked out from the time since `seen`, so a clock shoved ten days forward charges every being ten days of hunger it was never alive for. A test that sets the date proves a rule runs at that date; it does not prove the date is reachable by playing, and the suite owes one run that arrives by simulating.
