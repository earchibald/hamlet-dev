...
- `tests/types.js`: runs `tsc --checkJs` on `src/sim/` through `tsconfig.sim.json`, and fails on any error. It also runs `tsc` on `tsconfig.probe.json`, a fixture with one misspelt field on a Being and one on a Tile, and fails unless both are still caught, so a type this loose cannot come back unnoticed. Both checks skip, with a message that names what is not being checked, if TypeScript is not installed. `src/ui/` is not part of this test yet: see `tsconfig.ui.json` below. Fast.
