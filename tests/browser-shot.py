...
with sync_playwright() as p:
    ...
    pg.keyboard.press('l'); pg.wait_for_timeout(200)
    print('tool hint:', pg.inner_text('#hint'))
    tree=pg.evaluate("(()=>{for(let x=0;x<W;x++)for(let y=0;y<H;y++){const q=hasTile(x,y,0)&&tileAt(x,y,0);if(q&&q.feature==='tree'){const s=secOf(x,y);return {x:x,y:y,sx:s.sx,sy:s.sy};}}return null;})()")
    print('tree tile', tree)
    pg.evaluate(f"goto({tree['sx']},{tree['sy']}); setLevel(0)"); pg.wait_for_timeout(300)
    box3=pg.query_selector('#map').bounding_box()
    pg.mouse.click(box3['x']+((tree['x']-tree['sx']*28)+.5)*box3['width']/28, box3['y']+((tree['y']-tree['sy']*20)+.5)*box3['height']/20)
    pg.wait_for_timeout(300)
    print('strike hint:', pg.inner_text('#hint'))
    print('chronicle first line:', pg.inner_text('#chronicle').split('\n')[0])
