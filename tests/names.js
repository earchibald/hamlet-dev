...
/* Important review finding on task 2: a bare source word used to be struck to zero and dropped,
   ...
   BEHIND LONG=1 by the merge back, 2026-09-20, and this one test is the whole reason `npm run fast`
   missed its 300 s ceiling. MEASURED: `tests/names.js` alone costs 411.6 s, of which THIS TEST IS
   371.8 s. The next slowest test in the file is 4.1 s. `node --test` runs the suite's files in
   parallel, so the gate's wall time is its slowest single file, and its slowest single file was this
   one test. Moving it took the whole suite from 434 s to the figure in
   design/reports/2026-09-20-g4-the-merge-back.md.

   45 world days at 8.3 s a world day, which is what 371.8 s over 45 days comes to. That rate is
   nearly three times the 2.9 s a world day `slow()` quotes, because the rate rises as the valley
   fills and seed r is carrying a grown camp by day 45. So the skip is written out here with the
   measured figure rather than taken from `slow(45)`, which would have said 131 s and understated it
   by 2.8 times.

   THE DAY COUNT IS UNTOUCHED. Day 45 is not a round number here: it is the day on which the sky's
   word beats the people's word on seed r, which is the whole tie-break this test exists to hold. A
   shorter run would be a different test wearing this one's name. */
const BARE_WORD_SLOW = process.env.LONG || process.env.SLOW ? false
  : 'behind LONG=1: 45 world days on seed r, 372 s measured on this branch at 8.3 s a world day, which is 90 percent of this file\'s whole cost. LONG=1 runs it. The day count is untouched.';
test('a source word that would have won bare wins in its distinct form, measured on seed r', { skip: BARE_WORD_SLOW }, () => {
...
/* One 70-day run, shared by every test below that needs a long stretch of play. It is NOT a whole
   ...
   own `the camps grow` asserts under LONG=1 and which passes. A run of that length cost about twenty
   seconds when a world day was 0.31 s; at this clock it costs at least 168 s, at the 2.4 s a world day
   task 4 measured on alpha, and more as the valley fills. Corrected by the merge back, which measured
   8.3 s a world day on seed r at day 45. Task 4 rewrote the season clause of this comment and left the
   seconds standing, so the number outlived its own correction. The run is made once and kept, and
   every test that reads it is behind LONG=1. Seed alpha is the one of the
