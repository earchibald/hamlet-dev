...
- [ ] **The bless conditions, all six.** (1) Repoint `tests/soak.js` to `soak-golden.json` first, and say what happens to the working record. (2) Bless at seventy days on six seeds, with the exact command and the wall-clock cost in the report — the header says about 67 minutes a seed, so the user sees that number before it is spent. (3) Every field written by that run; hand-edit nothing; no value survives because it was already there, and that holds for every field, not only for `tiles`, because every field in this branch's golden is dev's. (4) Report which assertions were inactive during the blessing run. (5) Say whether the golden has seen a winter, and by which road, simulated or date-set. (6) dev-coordinator diffs `tests/soak-golden.json` against this branch's own HEAD at the gate. (The user's grant of 2026-09-20, with dev-coordinator's conditions; faults 1 to 3 found and verified on 2026-09-20.)
