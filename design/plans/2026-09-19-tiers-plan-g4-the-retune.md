...
| 3 | A being's head off the tick: state from elapsed time, a cheap proximity pass each tick | the same, and a world day in under 5 s. **Day 3 met: 2.99 to 3.40 s across five runs, at one-minute load averages from 2.62 to 4.36, against 9.86 s before. Day 50 missed at 17.98 s with 82 people.** |
...
| 8 | A run that reaches winter now costs 274 days where it cost 25. What does the suite do? | "We can set the date directly if we need to." A test may jump the clock to a season instead of paying the days. The year stays 365 and the seasons stay 91/91/91/92. |
...
#### What ruling 8 obliges, because a date-set test proves less than an arrival

A test that sets the day to 300 proves that a winter rule runs on day 300. It does not prove that a
world can arrive at day 300 by playing. Those are two claims and the suite holds both, or the
arithmetic that carries a world from spring to winter is untested again in a new way:

- **At least one run arrives by simulating.** It crosses a season's turn, and it may be the only slow
  run in the suite. It is behind `LONG=1` if it must be, but it exists and it is named.
- **Every other seasonal test sets the date**, and says in its own words that it sets it.

Setting the date makes a new writer of the clock, so:

- **One function does it, it has a name, and it lives in `tests/lib/run.js`.** Not in `src/sim/`. The
  engine gains no way to be shoved to a date, because a rule reachable only through a back door in
  the product is a rule the product can lose without a sound.
- **The function is written down in `CLAUDE.md`** by the task that adds it, beside the rule it bends,
  and it names the one thing it writes.

Task 10 owns the work. Task 11 checks the two claims are both held and says in the bless report which
road the golden's winter came by, if it came at all.
...
- [x] Budget test: a world day on seed `r` at day 3 in under 5 s. Report seconds for day 3 and, from the long run, day 50. **Day 3 met. It is a range and not one number: 2.99 to 3.40 s across five runs, at one-minute load averages from 2.62 to 4.36, against dev's 9.86 s. Method: the budget test's own harness in `tests/beings-lazy.js`, three world days on seed `r` in Node, the third day timed. The five are 3.05 s at load 4.36 and 3.33 s (the implementer), 3.40 s at load 3.86 (the task 3 review), and 2.99 s at load 2.62 and 3.28 s at load 3.29 (the task 3 remediation). The budget is 5 s and every run meets it. The test asserts only at a one-minute load average of 2 or below and otherwise skips loudly, naming the load average, so each figure above is a measurement and not an assertion. Day 50 MISSED: 17.98 s, with 82 people against day 3's 2. Seconds a person a world day is flat from day 25 on, so the day-50 miss is the valley filling and not the body; the report says so and does not explain it away. The last 2.3 s of the day-3 win came from `threatsFor`, which ruling 6 runs for every being on every tick.**
...
- [ ] **Report how often the horizon was pinned, and why.** Asked for by the user through dev-coordinator. Every other figure in this task is the result; this is the cause, and it is the only one that says whether the result generalises. The skip's speed is two facts multiplied: how cheap a jump is, and how often you are allowed one. Reporting the product alone makes a structural win and a lucky seed look the same. A world day at 0.4 s with 5 percent of ticks pinned holds as the valley fills; the same 0.4 s with 60 percent pinned is an artefact of a quiet seed and collapses on a seed with wolves near camps, which is the seed a long run exists for. The floors turn on whether long runs are affordable in general.
  - The percentage of ticks pinned, per seed, over the runs this task already measures.
  - **Split it by why, and leave a bucket for none of the above.** A burning tile and a predator near a sleeping or working person have different futures: a camp's fire is its normal state, so a fire-dominated pin is a permanent cost, while predator proximity is episodic. Count any third cause separately rather than letting it fall into one of the two. A two-way split silently absorbing a third case is this plan's recurring fault, and here it would make a permanent cost look episodic.
...
- [ ] **`strides()` is wrong for every value this task owns, and the sprite block with it.** Read `design/reports/2026-09-19-g4-the-stride-that-was-one-tick.md` before touching a row: it holds the arithmetic, the fourteen values and the control. There is no issue for this and there will not be one — the defect is branch-local, because `strides` and `ticks` are the same function on `dev`, so nothing in the release is wrong and the repo's issues are triage for `dev`. This checkbox is the only owner. (patcher's finding; verified in both trees.) `strides()` is `n => Math.round(n * 172.8)`, which is `2 x 86.4`, and it encodes one claim: a task stop was two old ticks. dev's gate at `src/sim/beings.js:394` reads `if (!fast && (tick + a.id) % sp.stride) return;`, so the claim fails twice over. A `fast` task never met the gate, whatever its species. A sprite carried `stride: ticks(1)` at dev `src/sim/species.js:11` where every other species carried `ticks(2)`. Both accrued one progress a tick, so both come out at twice their old world meaning. Doubled: `chase.spearSprite` (90), `chase.wolf` (140), `chase.wolfPerSkill` (30), `chase.stalk` (160), `chase.guard` (200), and the five sprite durations `dance`, `danceSeen`, `mossEvery`, `visit`, `visitGift`. Halved, being rates on the same tasks: `danceGlows`, `danceRests`, `dancePlays`, `visitPlays`. The conversion is `ticks()` and `tickRate()`. **Not affected, and the check that the rule is not overfitted:** `chase.deer`, `deerPerSkill` and `deerMissed` are correct, because the deer hunt declares `fast: false` at `src/sim/tasks.js:392` and really did meet the gate. Write the failing test first — assert a value against its dev world meaning and watch it go red here — because a doubled duration blessed into the working record as a balance change is the fault this branch exists to catch.
- [ ] **Measure maulings against the doubled chase limits, not only against today's numbers.** The five `fast` limits above let a predator press a chase twice as long, which changes how often a hunt ends in a kill. The mauling line below compares against today; today is already wrong. Take the dev figure as the reference.
- [ ] **The sprite gets no movement advantage back** (patcher's ruling, settled, not an open question). Every species walks one tile a tick. The sprite alone carried `stride: ticks(1)` on dev, so it was the one thing moving at double speed, and the plan flattened that without noticing. It does not come back: the one encounter where the speed could matter is the spear chase, and both sides of it carry `fast` (`src/sim/fae.js:4`, `src/sim/beings.js:228`), so the double step never applied where it counted. Restoring it would also spend the column's one meaning — 2 is a run — on a species that is not running, and leak into wander, forage and collect, none of which was ever a run. The sprite's character is its needs, its hours and its glow.
...
- [ ] **The ladders must stay the same length, and nothing says so** (issue #85). `SPEEDS` gains a fifth rung; `PACES` has four. Three couplings tie them, and none is obvious from the edit: every speed button in `src/page.template.html:207` carries both `data-speed` and `data-pace`; one handler at `src/ui/main.js:47` picks between them by era, so a new speed button is live in the gods era too; and `src/ui/keys.js:102` builds the key map from `SPEEDS` while indexing `PACES` by the `SPEEDS` index. The third does **not** throw — `PACE_LABEL[undefined]` is a miss, so the key row ships reading "Pace undefined". Decide whether `PACES` gains a rung with `SPEEDS` or `keys.js:102` stops indexing one ladder by the other's index, and say which.
- [ ] **Budget a third edit for `tests/ui.js` if both ladders grow.** Dev `61a9063` states the relation and closes the trap, so the signals above are no longer the ones you will meet. Two assertions now guard the walk, and `assert.equal(api.SPEEDS.length, api.PACES.length, ...)` sits above `const SHIFTED = '!@#$'`, so it speaks first. Grow `SPEEDS` alone and the length assertion fires, naming `keys.js:102`. Grow both together and the length assertion passes, correctly, and `assert.ok(api.SPEEDS.length <= SHIFTED.length, ...)` fires instead: the test types the shifted digit for each rung and knows four characters. That ceiling is the test's own limit, correctly labelled as such, and not a rule of the interface — extend the string, do not narrow the ladder to fit it. Both cases were measured on dev, not read. (dev-coordinator, verified here at `61a9063`.)
- [ ] **Three limits live in `src/ui/keys.js:102` and nothing relates them.** The rung counts must match each other; the test's shifted-digit string stops at four; and the key space stops at nine, because the line builds its key as `String(i + 1)`, so a tenth rung asks for the key `"10"`, which no `KeyboardEvent` produces. That binding would be silently dead: no throw, no red test. So "extend the string" is right up to nine rungs and wrong at ten. Task 8 wants five and is far inside the bound. Two supports for the nine: `keys.js:103` already walks `for (let n = 1; n <= 9; n++)` for the drawer rows, so the digit space is written down one line below and was never related to the line above it; and Shift+5 is free — dev-coordinator grew both ladders on dev, built the map, and found no duplicate `(key, shift, alt, focus)` tuple anywhere, with `Alt+Shift+5` (`muteMenu`) a different tuple. A fifth rung needs no key rearrangement.
- [ ] **Do not trust the failure signals here.** A `SPEEDS` edit reddens the key-map test by the button label; a `PACES` edit does not (both halves verified by planting). The `TypeError` seen at `tests/ui.js:1410` is the test's own `'!@#$'[i]` running off the end of a four-character string, not the index into `PACES` — it is convincing and points at the wrong line. Issue #80 deletes the `SPEEDS` restatement that fails first, so after it lands the misleading signal is the only one left.
...
**Files:** `tests/names.js` (**added**: see the first checkbox), `tests/crafts.js`, `tests/closing.js`, `tests/gnomes.js`, `tests/dwellers.js`, `tests/wanderer.js`, `tests/snapshot.js`, `tests/door.js`, `tests/tasks.js`, `tests/terrain.js`, `tests/settle.js`.

- [ ] **`tests/names.js` joins this task, and its three live failures are triaged here** (dev-coordinator holds the G4 PR on this). The plan never named `tests/names.js`, so under `SLOW=1` it has three failures belonging to nobody: 72 pass, 3 fail. Running a file is not the same as owning what it says — running proves the code was exercised, ownership is what turns a failure into a decision. The three: (a) `tests/names.js:933` hands `buildHut` `progress: 9999` where `CLOCK.work.hut` is now 20736, and the failure surfaces as a `TypeError` on the line after; set progress from the task's own cost in `CLOCK.work` rather than from a larger guess, and see the report's `progress: 9999` section, because `setSnare` has under a factor of two in hand. (b) The day-70 layout guard compares against `tests/names-layout.json`, frozen on dev at `aa3a426` before this branch existed. **Re-taking that record is bless-class and is not this task's to do.** Its message is fixed already. (c) `by day 40 every person with ten days in a camp has an epithet` fails with `Esk has no epithet` and is **unexplained**; `CLOCK.names.epithetAfter`, `nameTick`'s gate and `CLOCK.names.nameHour` were each compared across both branches and each is identical. Do not close it with the population reading unless it is measured.
- [ ] **`tests/wanderer.js` is red with three failures, and the day numbers are not the fault.** Read `design/reports/2026-09-20-g4-the-suite-never-reaches-winter.md` first. `winter blocks the wanderer until spring` (`:89`), `winter blocks the founder until spring` (`:247`) and `a line begun out of the hills does not start another while it lives` (`:269`) all fail because the tests carry dev's 32-day year in their own comment — "Days 25 to 32 are winter" — and a year is now 365 days. Day 28 is spring here. `afterTheLast` at `src/sim/camps.js:182` is byte-identical to dev's, `isWinter()` included, and is behaving correctly. The same file on dev is 18 pass, 0 fail, run as a control. Do not move the day numbers to make them green: that converts a real gap in coverage into a passing test, which is the fault this branch exists to catch.
- [ ] **The winter question is answered by ruling 8: set the date.** Read `design/reports/2026-09-20-g4-the-suite-never-reaches-winter.md` first, for what went quiet and how. No test on this branch simulates one winter day: the longest run anywhere is 70 days and winter begins on day 274, so fifteen `isWinter()` call sites across seven files are unreachable by the whole suite, the soak included. A 70-day run crossed 16 winter days on dev and crosses none here. The year stays 365. This task builds the date-setter named under "The calendar", uses it for the three red tests in `tests/wanderer.js`, keeps one run that reaches a season by simulating, and writes the setter into `CLAUDE.md`. A day number moved to make a test green, with no date set and no arrival, is the fault this branch exists to catch.
...
- [ ] **Six `tiles` values in the golden arrived by merge and were never measured on this branch. Say so, or the bless presents dev's numbers as G4's.** The G4 golden at HEAD `dabe8a3` is md5 `f2b8a275…`; the staged dev merge takes it to `1e1248d4…` by adding one `"tiles"` field per seed — `1c7fc9d5`, `a357b1ab`, `8ecc23ba`, `23a2a58d`, `b1a7f69d`, `35773a8a` — which are `origin/dev`'s values byte for byte. The retune changes the world, so this branch cannot reproduce them: they are six assertions false by construction, sitting in a blessed record that nobody blessed. No existing fingerprint moved; the change is purely additive. **The standing rule held in the letter and failed in the thing:** nobody ran `UPDATE_GOLDEN=1`, and a merge reached the same file by another road. The rule names a command; what it protects is the file. So this task's bless is mandatory rather than optional, for a reason unrelated to the retune, and the report must name these six as merged-in and unmeasured. dev-coordinator holds the PR if the report presents them as blessed numbers.
- [ ] **The record the user blesses has never seen a winter. Say so in the report, in the report's own words, before they bless it.** Read `design/reports/2026-09-20-g4-the-suite-never-reaches-winter.md`. The soak runs 70 days; winter begins on day 274; a 70-day run crossed 16 winter days on dev and crosses none here. So fifteen `isWinter()` call sites across seven files can be changed, broken or deleted by any task from 3 to 11, and the six-seed record will agree with itself afterwards. This is not an argument for changing the bless. It is that "the golden agrees" will otherwise mean something to the user that it does not mean, and the blindness is new since task 1 rather than a standing limit of the record.
- [ ] **The bless is granted, and `UPDATE_GOLDEN=1` alone does not perform it.** Three mechanical faults sit between the command and the record, each verified here rather than read. The bless is a procedure, not a command: repoint, run, verify the blessed file changed, and say what became of the working one.

  **Fault 1: the command writes the wrong file.** `tests/soak.js:62` sets `GOLDEN` to `soak-working.json`; dev's line 21 sets it to `soak-golden.json`. Lines 152 and 249 write `GOLDEN`. So the bless as first stated would leave `tests/soak-golden.json` untouched — still dev's record, blessed in name only, with the branch's real numbers in a working file. Step one is to repoint line 62 and to rule on the working record: deleted, or kept as the everyday short-run record.

  **Fault 2: a six-seed seventy-day run writes nothing, and reports green.** `isDefault` at line 58 is `!LONG && DAYS === DEFAULT_DAYS && SEEDS.join() === DEFAULT_SEEDS.join()`. Both the comparison (line 150) and the write (line 249) carry `skip: !isDefault`. So `LONG=1 SEEDS=r,x,alpha,beta,gamma,delta UPDATE_GOLDEN=1` skips both: about 6.7 hours spent, zero fields written, exit code 0. **Measured, not read:** `UPDATE_GOLDEN=1 SEEDS=r DAYS=1 node tests/soak.js` prints `﹣ write the golden record # not the default run` and leaves `tests/soak-working.json` byte-identical, md5 `3841f72f…` before and after. There is no environment combination that blesses seventy days: `DAYS=70` alone also fails `DAYS === DEFAULT_DAYS`. Only the three-day default can write, so a plain bless would freeze `days: 3` as the blessed truth and turn "the floors three days cannot hold" from a branch condition into the record.

  **Fault 3: the run that can write and the run that gates are different runs.** The two six-seed floors (`the six camps together grow`, `the far countries are reached`) need `LONG && SIX_SEEDS`; `the camps grow` needs `LONG`. None of them is active in the only run that can write a record. So the write condition must be re-expressed as "this run is the record's run", not "this run is the default run", and the record must carry its own shape — its days and its seeds — so a seventy-day record is never silently compared against a three-day run. The likely shape is two records with two comparisons, the short everyday one and the blessed long one; the snapshot oracle at line 168 reads a golden line for seed `x` and needs one that matches its own length, so decide it there rather than discover it.

- [ ] **`UPDATE_GOLDEN=1` must refuse out loud, and that is the one part of the bless that tests itself.** The durable fix is not to find the flag combination that writes. Nobody types `UPDATE_GOLDEN=1`, waits 6.7 hours, and wants a skip. `skip: !isDefault` is right for the COMPARISON and wrong for the WRITE, and that is the seam: guard the write by "this run is the record's run", and when it is not, fail and name the clause that refused — `LONG`, `DAYS`, or `SEEDS`. **The refusal is checked twice, and the early check is the one that matters.** A failure at the write, line 249, arrives after the six seeds have run: a better exit code, 6.7 hours late. So evaluate the predicate once at load, before the first seed, where a wrong flag costs a second, and keep a guard at the write as a backstop, because a later edit can move the seeds without moving the load-time check. Drive both red first: plant a wrong flag, run the bless, watch it refuse in the first second. Then keep the refusal as a test case, so a bless that cannot write is a gate rather than something rediscovered in six months. The one-day proxy is the instrument: `UPDATE_GOLDEN=1 SEEDS=r DAYS=1` costs a minute and exercises the same predicate as the long run.

- [ ] **The bless conditions, all eight.** (1) Repoint `tests/soak.js:62` to `soak-golden.json` first, and say what happens to the working record. Not before task 11: every task between here and there answers to `soak-working.json`, and repointing early strands them. (2) **Make a seventy-day six-seed bless possible, then perform it.** That command does not exist on the branch today, so the condition is not "run it" — what was changed to make it possible is part of the report, and the change is driven red. (3) Every field written by that run; hand-edit nothing; no value survives because it was already there, and that holds for every field rather than for `tiles`, because every field in this branch's golden is dev's. (4) Report which assertions were inactive during the blessing run. (5) Say whether the golden has seen a winter, and by which road, simulated or date-set. (6) dev-coordinator diffs `tests/soak-golden.json` against this branch's own HEAD at the gate. (7) **The record states the days and the seeds it was measured at, and a run whose shape does not match the record it is compared against fails rather than passes.** Otherwise a seventy-day record silently gates a three-day run. Decide the shape in writing — two records with two comparisons is the likely answer, and the snapshot oracle at line 168 needs a record of its own length. (8) **Prove the write happened, and that the run measured it.** `md5` the record before and after, both in the report. Two hashes prove the file changed; they do not prove the run measured what is in it, because a hand-edit moves the hash the same way. What separates them is that a blessed record reproduces, so run the same command again and show it green against the record the first run wrote. **One seed is enough** (dev-coordinator, settled: they take the bless report as evidence and will not re-run it). A record that fails to reproduce fails on any seed, and paying 6.7 hours twice to learn the same thing is a bad trade. Name the seed and why it was chosen. The wall-clock cost goes in the report too, and the user sees it before it is spent: about 67 minutes a seed by the file's own header.

  (The user's grant of 2026-09-20. Conditions from dev-coordinator; faults 1 to 3 verified here, and the no-op measurement is this branch's.)

- [ ] **The suspended files become public skips.** PR #100 puts the fast suite in CI on every push to dev and every pull request into dev. It reports and does not gate the release. So when G4's execution PR opens, the six files task 1 suspended show as skips in a public log, with their suspension notes as the reason. That is the notes doing their job, and it is a second reason the notes must say the true day counts: `tests/wanderer.js` asks 493 days across 15 runs, not 48.
- [ ] **Expect issue #94 to arrive red.** A soak assertion that a season was reached is approved and being implemented on `dev`. It fails on G4 until this branch's seasonal work lands. That is the test working, not a fault on the branch.
...
- [ ] **The six-seed seventy-day run. OWED, not offered. The pull request does not open without it.**
  `LONG=1 SEEDS=r,x,alpha,beta,gamma,delta node tests/soak.js`. Run it **as late as possible**, after
  the last task that can move population — schedule it late for coverage, not for cheapness, even
  though after task 4 it may be minutes rather than hours. This is the **only** place in G4 where the
  two cross-seed floors are evaluated at all: the default soak is three days and skips them, and the
  `LONG=1` gate is one seed. A uniform sag across every seed is caught by nothing else. **A red is a
  finding to diagnose, never a number to update.** Record the numbers in the PR body, green or red:
  `sums.humans` and `sums.born` against **180** and **15**, and the four far-country sums against
  **2, 1, 5, 1**. Dev measured 255 and 37, and 8, 5, 15, 4.
