...
- [ ] **The bless is granted, on one condition: every field is written by a G4 run.** Run `UPDATE_GOLDEN=1 node tests/soak.js` on this branch and let the record be written. Do not hand-edit `tests/soak-golden.json`, and do not let a value stand because it was already there — the six `tiles` hashes above are the case in point. The bless report states three things: (a) that those six arrived by merge and were never measured here; (b) which fields the bless changed, and by how much, `tiles` among them; (c) whether the golden has now seen a winter, and by which road, simulated or date-set. dev-coordinator diffs `tests/soak-golden.json` against this branch's own HEAD at the gate, not against the working tree or the index. (The user's grant, relayed by dev-coordinator, 2026-09-20.)
