...
- **The camp and its events.** The founder's camp is named the moment the site is chosen, by `chooseSite` or by the `setSite` fallback for a site set with no founder yet (`nameFoundersCamp`). The hearth pass, `nameCampAtHearth`, runs from the nightly pass `nameTick`, once a day at `CLOCK.names.nameHour`. Events are named at the fire the next night, from the tags `log` carries and `CLOCK.names.eventMemory`. The tags `old`, `deer`, `fish`, and `pot` carry no row in `EVENT_NAMES` yet, so none of them names a night.
...
- **Epithets, fates, and lineage.** Everyone earns an epithet after `CLOCK.names.epithetAfter` days in a camp, scored from opinions, deeds, and birth; the camp is the namer, so no trait weighs it. A later candidate must score half again as much to replace one already held. Fate gives the last epithet at death. `fullName(a)` reads a person's epithet; a god keeps its own epithet from `gods.js` and never gets one from naming work.
