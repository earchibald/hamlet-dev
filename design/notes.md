...
- **The camp and its events.** `nameFoundersCamp` names a founder's camp. It runs the moment the site is chosen, from `chooseSite` or from the `setSite` fallback for a site set with no founder yet. It also runs from `startFoundCamp`, when a party sets out, before that camp has any site at all. A camp whose text is already taken asks the namer instead, because the name pool is recycled and two living people can share a name. A camp with no name record is "the camp" in a line; `campNameOf(c)` reads the record, never the text. The hearth pass, `nameCampAtHearth`, runs from the nightly pass `nameTick`, once a day at `CLOCK.names.nameHour`. Events are named at the fire the next night, from the tags `log` carries and `CLOCK.names.eventMemory`. A night is named from the event table alone, never from a place: `nameThing` builds the pool from `eventCandidates` for a night, the way it builds it from the lore alone for the valley. The table offers fourteen texts, and once they are spent a night keeps no name. The rule runs the other way too. An event offers a place only its joined word, such as `Ashhill`. The phrase, such as `the Night of the Wolf`, is the name of a night, so `eventCandidates` gives it to an event alone. The tags `old`, `oldCold`, `deer`, `fish`, and `pot` carry no row in `EVENT_NAMES`, so none of them names a night.
...
- **The village and the valley.** A camp names itself again on becoming a village, with its own current name as a candidate at 60, so most villages keep the name and gain only the title. The valley is named once, by the first camp in list order that is a village, from the lore words alone, at a base of 40. Three fallback names, `Vale of ...` and `... Vale`, sit at a lower base of 20, so they win only once every lore word is taken elsewhere.
