...
- The map fills the rest. Four views: sector at 26 px, nearby at 9 px, world at 3 px, and camp fire at 26 px. M cycles them in that order. The camp fire view is the sector view's shape, with the chosen camp's pit in the middle, or its site before the pit is built. It stops at the world's edges. With no camp to centre on, M skips it. A cursor that leaves it, or a sector step, opens the sector view. `locOrigin` in `derive.js` is the one origin that the drawing, the pointer, the cursor, and the tools read for both close views. The tools and the view buttons float top left. The foot shows the newest chronicle line when the chronicle drawer is shut.
...
- Every drawer starts closed. A tab's own key, or a click on the tab, opens or closes it. Neither moves the keyboard focus off the map. The game's own keys keep working while the card sits open. A click inside the card gives the card the focus. Esc gives the focus back to the map. The card stays open. Make world and Take a god always start with every tab closed, even when a saved session had one open. Continue and a loaded save keep the tabs that save had open. A search jump (`/`) or a stage jump (the goal chord) still takes the player into the card on purpose. The player asked to see one particular thing there.
- The user accepted one cost on 2026-09-21. Until the player clicks into a card, the keyboard cannot reach the card's own controls. This breaks the rule that everything has a hotkey. The user allows it for now. A tab's hotkey must close the tab again. The game's own keys must keep working while a card is open. One key cannot both hold the keyboard for the card and hand it back to the game.
