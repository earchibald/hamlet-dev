...
The days open at 8x. At 1x the first fire pit took about 4 real minutes, and at 64x a chronicle line is gone in about a second. A speed the player picks with a key, a button, or the palette is kept, and it saves a marker, `speedChosen`. Older pages saved speed 1 on every load, with no marker. So an old record of 1 with no marker opens at 8x, but an old 8, 64, or 256 is kept, because only a click could save one. A load keeps the speed the player picked last.
