...
- **Grace** starts at 30, so that the first fire prayer can be answered. At 0, the first prayer was always silent: the founder had earned only 9 grace by dawn, and a Spark costs 15. Grace comes from belief at the rate `CLOCK.faith.grace`, and it caps at 100. The first income rate left grace at the cap for most of an eight-day run, so it was halved.
