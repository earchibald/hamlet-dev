...
| Sources at the door | mythos, section 0 | hook | player built: light and poke. chance, llm, human reserved in DOOR_SOURCES. The log is api.doorLog; api.replay is { seed, options, log }. An event stamped tick N was applied after step N. An LLM source cannot answer inside a tick, so it needs a queue drained at a fixed point in the step order; plan for it when the source arrives. |
