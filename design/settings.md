...
| Save and load | G, time and tiers, section 3 | hook | Snapshots through a table of reference fields: `takeSnapshot()` and `loadSnapshot(snap)` in `src/sim/snapshot.js`. `load` passes the door: `inject({ source, act: 'load', snapshot })` replaces the world and logs a bare entry, and a replay passes over it. Save to a file and the autosave slot are not built yet. |
