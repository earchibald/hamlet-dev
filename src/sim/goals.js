...
const GOALS = [
  ...
  { id: 'firepit', title: 'Build a fire pit', stage: 'fire', after: 'camp', need: { rock: 6, stick: 8 },
    ...
    offers(a){
      ...
      if (camp.stash.rock >= n.rock && camp.stash.stick >= n.stick) out.push({ label: 'build the fire pit', score: 60, start: a => startBuild(a, camp.site, CLOCK.work.firepit, 'Building the fire pit', a => {
        if (camp.pit) return;
  ...
  { id: 'fire', title: 'Keep the fire burning', stage: 'fire', after: 'firepit', standing: true,
    ...
    offers(a){
      ...
      if (p.lit && p.fuel < PIT_MAX * 0.6 && (camp.stash.stick > 0 || camp.stash.log > 0)) out.push({ label: 'feed the fire', score: p.fuel < PIT_MAX * 0.25 ? 85 : 50, start: a => startBuild(a, camp.pit, CLOCK.work.feedFire, 'Feeding the fire', a => {
        if (!p.lit) return;
      ...
      if (!p.lit && p.fuel > 0 && camp.tools.firestones && !(a.cooldown['strike sparks'] > tick)) out.push({ label: 'strike sparks', score: 80, start: a => startBuild(a, camp.pit, CLOCK.work.strikeSparks, 'Striking sparks into the tinder', a => { if (p.lit || p.fuel <= 0) return; if (rng() < 0.2 + a.skills.craft * 0.1 + a.traits.patience * 0.25){ p.lit = true; camp.everLit = true; log(`${a.name} coaxes a spark into flame. The fire is back.`, campHumans(), 'good'); for (const h of campHumans()) addThought(h, 'hearth', 'The fire is lit', 8, CLOCK.thought.hearth); gainXp(a, 'craft'); } else { addThought(a, 'sparks', 'Sparks, but no flame', -3, CLOCK.thought.sparks); a.cooldown['strike sparks'] = tick + CLOCK.cooldown.sparks; } }) });
      if (!p.lit && p.fuel > 0 && camp.stash.moss > 0) out.push({ label: 'light the pit with glowing moss', score: 92, start: a => startBuild(a, camp.pit, CLOCK.work.mossLight, 'Blowing on the glowing moss', a => { if (p.lit || camp.stash.moss <= 0 || p.fuel <= 0) return; camp.stash.moss--; p.lit = true; camp.everLit = true; log(`${a.name} tucks the glowing moss into the pit and blows. The fire takes. No lightning, no sky.`, campHumans(), 'major'); for (const h of campHumans()) addThought(h, 'hearth', 'The fire is lit', 8, CLOCK.thought.hearth); }) });
      ...
      if (!p.lit && p.fuel <= 0 && camp.stash.stick >= 4) out.push({ label: 'lay the fire again', score: 60, start: a => startBuild(a, camp.pit, CLOCK.work.layFire, 'Laying the fire', a => { if (camp.stash.stick >= 4){ camp.stash.stick -= 4; p.fuel = 160; log(`${a.name} lays fresh wood in the pit.`, [a]); } }) });
      return out;
    } },
  ...
  { id: 'cook', title: 'Cook what we catch', stage: 'food', after: 'fire', standing: true,
    ...
    offers(a){ if (!pitLit()) return [];
      ...
      if (camp.stash.venison > 0) return [{ label: 'butcher and cook the deer', score: 62, start: a => startBuild(a, camp.pit, CLOCK.work.butcherDeer, 'Butchering the deer', a => { if (camp.stash.venison <= 0) return; camp.stash.venison--; stashAdd('cooked', 5); stashAdd('hide', 2); if (camp.rack) stashAdd('smoked', 4); gainXp(a, 'cook'); log(`${a.name} butchers the deer. Meat for days${camp.rack ? ', and strips on the rack' : ''}.`, campHumans(), 'good'); }) }];
      if (camp.stash.fish > 0) return [{ label: 'cook the fish', score: camp.stash.fish >= 2 ? 62 : 45, start: a => startBuild(a, camp.pit, CLOCK.work.cookFish, 'Cooking fish over the fire', a => { if (camp.stash.fish > 0){ stashTake('fish'); stashAdd('cooked', 2); gainXp(a, 'cook'); log(`${a.name} cooks a fish over the fire.`, [a], 'good'); } }) }];
      if (camp.stash.carcass <= 0) return []; return [{ label: 'cook the catch', score: camp.stash.cooked === 0 ? 55 : 35, start: a => startBuild(a, camp.pit, CLOCK.work.cookCatch, 'Cooking over the fire', a => { if (camp.stash.carcass > 0){ camp.stash.carcass--; stashAdd('cooked', 3); stashAdd('hide', 1); gainXp(a, 'cook'); camp.bestCook = Math.max(camp.bestCook || 0, a.skills.cook); log(`${a.name} cooks a rabbit over the fire and keeps the hide.`, [a], 'good'); } }) }]; } },
  { id: 'snare', title: 'Set snares for rabbits', stage: 'food', after: 'fire', standing: true, max: 4,
    ...
    offers(a){
      ...
      for (const s of snares){
        ...
        else if (!s.armed && camp.stash.stick >= 1) out.push({ label: 're-arm the snare', score: 40, start: a => startBuild(a, [s.x, s.y], CLOCK.work.rearmSnare, 'Re-arming the snare', a => { if (camp.stash.stick >= 1 && !s.armed){ camp.stash.stick--; s.armed = true; } }) });
      }
  ...
  { id: 'axe', title: 'Knap a stone axe', stage: 'tools', after: 'hearth',
    ...
    offers(a){
      ...
      if (camp.stash.rock >= 2 && camp.stash.stick >= 1) return [{ label: 'knap the axe', score: 58, start: a => startBuild(a, camp.stashTile, CLOCK.work.knapAxe, 'Knapping an axe', a => { if (camp.tools.axe || camp.stash.rock < 2) return; camp.stash.rock -= 2; camp.stash.stick--; camp.tools.axe = 1; gainXp(a, 'craft'); log(`${a.name} strikes flakes off a rock until it holds an edge, then binds it to a stick. The camp has an axe.`, [a], 'major'); addThought(a, 'axeMade', 'Made the first tool', 8, CLOCK.thought.axeMade); }) }];
  ...
  { id: 'shelter', title: 'Build a lean-to', stage: 'shelter', after: 'axe', need: { log: 4, stick: 10 },
    ...
    offers(a){
      ...
      if (camp.stash.log >= n.log && camp.stash.stick >= n.stick){
        ...
        return [{ label: 'build the lean-to', score: 56, start: a => startBuild(a, site, CLOCK.work.leanTo, 'Building the lean-to', a => { if (camp.shelter || camp.stash.log < n.log) return; camp.stash.log -= n.log; camp.stash.stick -= n.stick; const t = tileAt(...site); t.feature = null; t.struct = { type: 'leanto', camp }; camp.shelter = site; gainXp(a, 'build'); log(`${a.name} raises a lean-to beside the fire. The camp has a roof.`, humans(), 'major'); for (const h of humans()) addThought(h, 'roof', 'There is a roof to sleep under', 5, CLOCK.thought.roof); }) }];
      }
  ...
  { id: 'huts', title: 'Build huts', stage: 'shelter', after: 'shelter', standing: true, need: { log: 5, stick: 10, hide: 1 },
    ...
    offers(a){ if (!camp.shelter || camp.huts.length >= 4 || campHumans().length <= bedsFor()) return []; const n = this.need;
      if (camp.stash.log >= n.log && camp.stash.stick >= n.stick && camp.stash.hide >= n.hide){ const site = openSpotNear(camp.pit, 3, 6); if (!site) return []; return [{ label: 'build a hut', score: 48, start: a => startBuild(a, site, CLOCK.work.hut, 'Building a hut', a => { if (camp.stash.log < n.log || tileAt(...site).struct) return; camp.stash.log -= n.log; camp.stash.stick -= n.stick; camp.stash.hide -= n.hide; tileAt(...site).struct = { type: 'hut', camp }; camp.huts.push(site); gainXp(a, 'build'); log(`${a.name} finishes a hut. ${bedsFor()} can sleep under a roof now.`, campHumans(), 'good'); }) }]; }
      const out = [];
  ...
  { id: 'stone', title: 'Set an offering stone', stage: 'sprites', after: 'fae', need: { rock: 2 },
    ...
    offers(a){ if (!camp.fae.known || camp.stone || !camp.pit) return []; if (camp.stash.rock < 2) return [{ label: 'gather rocks for the offering stone', score: 30, start: a => startGather(a, 'rock') }]; const site = openSpotNear(camp.pit, 4, 6); if (!site) return [];
      return [{ label: 'set the offering stone', score: 42, start: a => startBuild(a, site, CLOCK.work.offeringStone, 'Setting the offering stone', a => { if (camp.stone) return; camp.stash.rock -= 2; tileAt(...site).struct = { type: 'stone', camp, offering: 0 }; camp.stone = site; log(`${a.name} sets a flat stone at the edge of the firelight, for the sprites.`, campHumans(), 'good'); }) }]; } },
  { id: 'offer', title: 'Leave gifts for the sprites', stage: 'sprites', after: 'stone', standing: true,
    ...
    offers(a){ if (!camp.stone || camp.fae.favor >= 60 || camp.stash.berries < 3) return []; const st = tileAt(...camp.stone).struct; if (st.offering > 0) return []; return [{ label: 'leave berries on the stone', score: camp.fae.favor < 0 ? 46 : 30, start: a => startBuild(a, camp.stone, CLOCK.work.leaveBerries, 'Leaving berries on the stone', a => { if (camp.stash.berries < 2) return; stashTake('berries', 2); st.offering = 2; addThought(a, 'giftLeft', 'Left a gift for the sprites', 3, CLOCK.thought.giftLeft); }) }]; } },
  { id: 'ward', title: 'Ward the camp', stage: 'sprites', after: 'fae', need: { stick: 6 },
    ...
    offers(a){ if (!camp.fae.known || camp.ward || !camp.pit || !pitLit() || camp.fae.favor > -10) return []; if (camp.stash.stick < 6) return [{ label: 'gather sticks for the ward posts', score: 28, start: a => startGather(a, 'stick') }]; const site = openSpotNear(camp.pit, 3, 5); if (!site) return [];
      return [{ label: 'set the ward posts', score: camp.fae.favor < -20 ? 55 : 30, start: a => startBuild(a, site, CLOCK.work.wardPosts, 'Charring and setting ward posts', a => { if (camp.ward) return; camp.stash.stick -= 6; tileAt(...site).struct = { type: 'ward', camp }; camp.ward = site; camp.fae.favor -= 10; log(`${a.name} sets charred posts around the camp. The sprites will keep their distance now.`, campHumans(), 'good'); }) }]; } },
  ...
  { id: 'waterskin', title: 'Sew a waterskin', stage: 'tools', after: 'cook', need: { hide: 2 },
    ...
    offers(a){ if (camp.tools.waterskin || camp.stash.hide < 2 + hideReserved()) return []; return [{ label: 'sew the waterskin', score: 54, start: a => startBuild(a, camp.stashTile, CLOCK.work.waterskin, 'Sewing a waterskin', a => { if (camp.tools.waterskin || camp.stash.hide < 2 + hideReserved()) return; camp.stash.hide -= 2; camp.tools.waterskin = 1; gainXp(a, 'craft'); log(`${a.name} sews two hides into a waterskin.`, [a], 'major'); }) }]; } },
  ...
  { id: 'rack', title: 'Build a drying rack', stage: 'food', after: 'firepit', need: { stick: 6 },
    ...
    offers(a){ if (!camp.pit || camp.rack) return []; if (camp.stash.stick < 6) return [{ label: 'gather sticks for the rack', score: 30, start: a => startGather(a, 'stick') }]; const site = openSpotNear(camp.pit, 2, 3); if (!site) return [];
      return [{ label: 'build the drying rack', score: 45, start: a => startBuild(a, site, CLOCK.work.rack, 'Building a drying rack', a => { if (camp.rack || camp.stash.stick < 6) return; camp.stash.stick -= 6; tileAt(...site).struct = { type: 'rack', camp }; camp.rack = site; gainXp(a, 'build'); log(`${a.name} lashes sticks into a drying rack beside the fire.`, [a], 'good'); }) }]; } },
  { id: 'smoke', title: 'Smoke meat for lean days', stage: 'food', after: 'rack', standing: true,
    ...
    offers(a){ if (!camp.rack || !pitLit() || (camp.stash.carcass < 1 && camp.stash.fish < 1) || camp.stash.smoked >= 8) return [];
      const out = [];
      if (camp.stash.carcass >= 1) out.push({ label: 'smoke a rabbit over the fire', score: seasonOf() === 'autumn' ? 66 : camp.stash.smoked < 4 ? 60 : 30, start: a => startBuild(a, camp.rack, CLOCK.work.smokeMeat, 'Smoking meat', a => { if (camp.stash.carcass < 1) return; stashTake('carcass'); stashAdd('smoked', 2); stashAdd('hide', 1); gainXp(a, 'cook'); log(`${a.name} hangs strips of rabbit in the smoke. They will keep.`, [a]); }) });
      if (camp.stash.fish >= 1) out.push({ label: 'smoke a fish over the fire', score: seasonOf() === 'autumn' ? 64 : camp.stash.smoked < 4 ? 58 : 28, start: a => startBuild(a, camp.rack, CLOCK.work.smokeFish, 'Smoking fish', a => { if (camp.stash.fish < 1) return; stashTake('fish'); stashAdd('smoked', 2); gainXp(a, 'cook'); log(`${a.name} hangs a fish in the smoke. It will keep.`, [a]); }) });
      return out;
    } },
  ...
  { id: 'scout', title: 'Found a second camp', stage: 'settlement', after: 'shelter',
    ...
    offers(a){ if (!camp.shelter || camp.sentParty || camps.length >= 6 || campHumans().length < 5 || tick - camp.founded <= CLOCK.party.campAge || !(seasonOf() === 'spring' || seasonOf() === 'summer') || a.traits.bravery < 0.5) return []; return [{ label: 'lead a party to a new valley', score: 40, start: a => startFoundCamp(a) }]; } },
