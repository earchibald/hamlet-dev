...
const GOALS = [
  ...
  { id: 'offer', title: 'Leave gifts for the sprites', stage: 'sprites', after: 'stone', standing: true,
    ...
    offers(a){ if (!camp.stone || camp.fae.favor >= 60 || camp.stash.berries < 3) return []; const st = tileAt(...camp.stone).struct; if (st.offering > 0) return []; return [{ label: 'leave berries on the stone', score: camp.fae.favor < 0 ? 46 : 30, start: a => startBuild(a, camp.stone, CLOCK.work.leaveBerries, 'Leaving berries on the stone', a => { if (camp.stash.berries < 2) return; stashTake('berries', 2); st.offering = 2; addThought(a, 'gift', 'Left a gift for the sprites', 3, CLOCK.thought.giftLeft); }) }]; } },
