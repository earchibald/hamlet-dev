...
function theLoneFounder(){
  ...
  /* Wait the whole stretch out again before the next attempt, rather than trying every tick. Restart
     the wait; do not clear it. `doomAt` carries two meanings and has no third. Zero means the wait is
     over and a founder arrived. A tick ahead means the wait is running. Nothing in it says "the wait
     ran out and nothing could be done", so this path has to borrow the second meaning. Clearing a
     variable on a failure path looks obviously right, and here it is wrong: zero sends the next tick
     back into the branch above, where the hermit is still alone beside a cold hearth, so the wait is
     set afresh and "This line ends with them" is said again, and again every ten days after. The
     chronicle says it once. */
  doomAt = tick + CLOCK.arrival.afterTheLast;
}
