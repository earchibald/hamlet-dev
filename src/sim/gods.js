...
/* The player leaves, or the god whose turn was open is gone. The open turn closes and the creation
   runs itself from where it stands: nobody is owed a wait for the next step, so the interrupted age
   is carried on to its end, or to the next thing there is to decide, right here. */
function releaseTurn(){
  pending = null;
  if (agePos) withGodRng(() => { if (ageDecide()) ageEnd(); });
  if (settleNow){ settleNow = false; settle(); }
}
