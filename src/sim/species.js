...
Object.assign(START, {
  ...
  scavenge(a){
    ...
    a.task = { type: 'eat', label: 'Going to a carcass', path: p, progress: 0, arrive(a, t){ if (!items.includes(found)) return 'fail'; t.label = 'Eating'; if (++t.progress < CLOCK.task.eatCarcass) return 'continue'; if (found.kind === 'venison' && rng() < 0.6){ a.needs.food = 100; return 'done'; } removeItem(found); a.needs.food = 100; return 'done'; } };
    return true;
  },
  ...
  huddle(a){
    ...
    if (!kin) return false;
    if (near(kin, a) <= 1){ a.task = { type: 'socialize', label: 'Chattering with kin', path: [], wait: CLOCK.task.kinChat, arrive(a){ a.needs.social = Math.min(100, a.needs.social + 30); kin.needs.social = Math.min(100, kin.needs.social + 20); return 'done'; } }; return true; }
