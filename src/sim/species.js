...
const SPECIES = {
  /* A person's needs, in points lost for each world hour. A need runs from 100 to 0 and urgency
     ...
     drinks, one sleep. The counts are the test, in `tests/tasks.js`.
       ...
       water 16    five drinks a day, measured on all six soak seeds: 15 or 16 over three days. A
                   drink fills the skin to 100, and a person puts thirst off until the work stops
                   paying, which measures out at about 70 points; five of those is 350, and 16 an
                   hour with one of the five taken in the night lands on it. Full to empty is six and
                   a quarter hours, which is less than a night, so a sleeper crosses `NEED_LOW` and
                   wakes to drink: see the wake in `bodyStretch`. A person near water is never in
                   trouble, and one cut off from it is in trouble inside the day.
  ...
  human:  { glyph: '@', label: 'human',  plural: 'people', decay: { food: perHour(7), water: perHour(16), rest: perHour(2.5), social: perHour(1), warmth: perHour(0) }, stride: 1, zmin: -2, zmax: 2 },
