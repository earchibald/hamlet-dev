...
/* Who is spawned for a making, and how. Read by paintCreatures. `n` is how many one making puts down. `most` is
   how many the whole valley may hold, over every making of that species. The making act is built to repeat: it
   makes what no country has, then fills every country of the god's pole that lacks a species of it. A long
   creation therefore marks the hunters country after country, and without `most` a valley took two wolves for
   each of twenty-six makings. The hunters are capped at the pair and the three the day era's own stray rule
   already keeps them near. The prey and the fae are not capped here. */
const SPAWN = { rabbit: { n: 6 }, deer: { n: 3 }, fox: { n: 1, den: true, most: 3 }, wolf: { n: 2, den: true, most: 2 }, sprite: { grove: true }, gnome: { burrows: true }, human: null };
...
function paintCreatures(first){
  groves = [];
  /* The species the cap turned away, and how many of each did come down. The order of `liveRegions()` and of
     `marksOf` fixes which countries get the hunters, so a seed always keeps the same ones. */
  const short = {}, came = {};
  for (const r of liveRegions()) for (const m of marksOf(r, 'making')){
    /* The roll records what the gods made. SPAWN decides only who a painter puts on the ground, so a making
       with no painter of its own, such as the people, still goes on the roll. */
    ...
    /* What the valley still has room for. A table with no `most` has no cap, and every making spawns in full. */
    const here = how.most === undefined ? how.n : Math.min(how.n, how.most - liveOf(m.value));
    /* The cap has bitten. Dig no den: nothing would live in it. */
    if (here <= 0){ short[m.value] = true; continue; }
    let placed = 0;
    if (how.den){ const c = digDens(within, m.value); if (c) placed = spawnInDens(m.value, here, c); }
    if (placed < here) spawnAnimal(m.value, within, here - placed, first);
...
function liveOf(sp){ let n = 0; for (const b of beings) if (b.alive && b.species === sp) n++; return n; }
