...
/* Shift with a digit sets a speed by its place on the ladder. Plain digits belong to the drawers and their rows.
   ...
   The two ladders are meant to be the same length, and tests/ui.js asserts it. When they differ, a
   row past the end of PACES has no pace to name. It then names the speed alone, because
   `PACE_LABEL[undefined]` is `undefined`, and the row printed the words `Pace undefined` to the
   player. This file runs while the page loads, so it does not throw on a short ladder: a throw here
   blanks the page, which is worse than a row that names one ladder. */
SPEEDS.forEach((v, i) => {
  const paceLabel = PACE_LABEL[PACES[i]];
  const label = paceLabel === undefined ? `Speed ${SPEED_LABEL[v]}` : `Pace ${paceLabel} \u00b7 Speed ${SPEED_LABEL[v]}`;
  KEYMAP.push({ key: String(i + 1), shift: true, focus: 'any', action: 'speedStep', arg: i, label, button: `speed${v}` });
});
